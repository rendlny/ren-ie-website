<div class="row">
  <div class="col-md-6">
    <script language="javascript" src="https://boardgamegeek.com/jswidget.php?username=RendlyTheFriendly&numitems=6&header=1&text=title&images=medium&show=recentplays&imagesonly=1&imagepos=left&addstyles=1&showplaydate=1&domains%5B%5D=boardgame"></script>
  </div>
  <div class="col-md-6">
    <script language="javascript" src="https://boardgamegeek.com/jswidget.php?username=RendlyTheFriendly&numitems=10&header=1&text=title&images=medium&show=top10&imagesonly=1&imagepos=left&addstyles=1&showplaydate=1&domains%5B%5D=boardgame"></script>
  </div>
</div>
<style>
  .BGGheader {
    font-size: 14px;
  }

  .BGGwrapper {
    max-width: 285px;
  }

  .BGGitem {
    max-width: 285px;
  }
</style>
